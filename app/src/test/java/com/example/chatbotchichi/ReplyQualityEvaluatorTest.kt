...
class ReplyQualityEvaluatorTest {
    ...
    @Test
    fun selectBest_penalizesServiceApologyBoilerplateAgainstManualExample() {
        val config = AutoReplyJson.defaultConfig("친구방").copy(
        ...
        val candidates = listOf(
            ReplyQualityEvaluator.evaluate(
                source = "primary",
                raw = "죄송합니다. 현재는 어려울 수 있으며, 필요한 사항이 있으면 안내드리겠습니다.",
                reply = "죄송합니다. 현재는 어려울 수 있으며, 필요한 사항이 있으면 안내드리겠습니다.",
                config = config,
                message = "지금 가능?",
                history = emptyList()
            ),
        ...
        assertEquals("human_style", best?.source)
        assertTrue(candidates.first().reasons.contains("service_apology_boilerplate"))
        assertTrue(candidates.last().reasons.contains("manual_example_match"))
    }
    ...
    @Test
    fun selectBest_penalizesGenericCasualAckAgainstManualExample() {
        val config = AutoReplyJson.defaultConfig("친구방").copy(
            roomStyle = "친한 친구방. generic 수긍만 하지 말고 상황에 짧게 반응. 예시: 아 빡세긴 하네"
        )
        val candidates = listOf(
            ReplyQualityEvaluator.evaluate(
                source = "primary",
        ...
        assertEquals("human_style", best?.source)
        assertTrue(candidates.first().reasons.contains("generic_casual_ack_boilerplate"))
        assertTrue(candidates.last().reasons.contains("manual_example_match"))
    }

    @Test
    fun selectBest_penalizesFalseDelayApologyAgainstManualExample() {
        val config = AutoReplyJson.defaultConfig("친구방").copy(
        ...
        val candidates = listOf(
            ReplyQualityEvaluator.evaluate(
                source = "primary",
                raw = "늦게 답해서 미안 아무것도 없긴해",
                reply = "늦게 답해서 미안 아무것도 없긴해",
                config = config,
                message = "오늘 뭐함",
                history = emptyList()
            ),
        ...
        assertEquals("human_style", best?.source)
        assertTrue(candidates.first().reasons.contains("false_delay_apology"))
        assertTrue(candidates.last().reasons.contains("manual_example_match"))
    }
    ...
    fun selectBest_penalizesTherapyEmpathyBoilerplateAgainstManualExample() {
        ...
        val candidates = listOf(
            ReplyQualityEvaluator.evaluate(
                source = "primary",
                raw = "그 마음 이해해. 많이 힘들었겠다.",
                reply = "그 마음 이해해. 많이 힘들었겠다.",
                config = config,
                message = "오늘 좀 빡세다",
                history = emptyList()
            ),
        ...
        assertEquals("human_style", best?.source)
        assertTrue(candidates.first().reasons.contains("therapy_empathy_boilerplate"))
        assertTrue(candidates.last().reasons.contains("manual_example_match"))
    }

    @Test
    fun selectBest_penalizesReactionSpamAgainstManualExample() {
        val config = AutoReplyJson.defaultConfig("친구방").copy(
        ...
        assertEquals("human_style", best?.source)
        assertTrue(candidates.first().reasons.contains("reaction_spam"))
        assertTrue(candidates.last().reasons.contains("manual_example_match"))
    }
    ...
    fun builtInScenarios_coverRequiredQualityCases() {
        ...
        assertTrue("no generic casual ack trait missing", "no_generic_casual_ack" in coverage.traitIds)
    ...
    fun builtInScenarioExamples_rejectMetaAndOverconfidentReplies() {
        ...
        val friendNoGenericCasualAck = ReplyQualityScenarios.builtIns().first { it.id == "friend_no_generic_casual_ack" }
        ...
        assertTrue(!ReplyQualityScenarios.evaluateReply("그렇구나", friendNoGenericCasualAck).passed)
        assertTrue(!ReplyQualityScenarios.evaluateReply("그럴 수 있지", friendNoGenericCasualAck).passed)
        assertTrue(!ReplyQualityScenarios.evaluateReply("늦게 답해서 미안 아무것도 없긴해", friendNoFalseDelayApology).passed)
