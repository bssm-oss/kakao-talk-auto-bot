...
object ReplyQualityScenarios {
    ...
    fun builtIns(): List<Scenario> {
        return listOf(
            ...
            Scenario(
                id = "friend_no_generic_encouragement",
                room = "친구방",
                sender = "민수",
                message = "오늘 좀 빡세다",
                config = AutoReplyJson.defaultConfig("친구방").copy(
                ...
                expectedTraits = setOf("casual", "short", "manual_example", "no_generic_encouragement"),
                minimumScore = 65
            ),
            Scenario(
                id = "friend_no_generic_casual_ack",
                room = "친구방",
                sender = "민수",
                message = "오늘 좀 빡세다",
                config = AutoReplyJson.defaultConfig("친구방").copy(
                ...
                expectedTraits = setOf("casual", "short", "manual_example", "no_generic_casual_ack"),
                minimumScore = 65
            ),
            Scenario(
    ...
    fun traitScore(reply: String, scenario: Scenario): Int {
        ...
        if ("no_generic_encouragement" in scenario.expectedTraits) {
            if ("generic_encouragement_boilerplate" in candidate.reasons || "helper_followup_boilerplate" in candidate.reasons) {
                score -= 45
            } else {
                score += 20
            }
